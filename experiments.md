...
* **Time Components:** Year, Week of Year, and Season (Spring, Summer, Fall, Winter).
* **Holiday Context:** `is_year_end`, weeks until the next holiday, and weeks since the last holiday.
...
* **Flags:** `abnormally_hot` (Z-score > 2) and `abnormally_cold` (Z-score < -2).
...
* **Focus:** We will monitor if high-value items (electronics) drop while daily essentials remain stable.
