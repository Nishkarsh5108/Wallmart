...
Our goal was to predict weekly sales for Walmart stores using historical data. Based on our experiments (detailed in `experiments.md`), we adopted the following key strategies:

*   **Feature Engineering:**
    *   **Target Encoding:** We replaced the categorical `Store` ID with `Store_Target_Mean` (average sales per store from the training set) to capture the "baseline" popularity of each location without creating sparse one-hot vectors.
    *   **Lag Features:** We created `Lag_Sales_1w` (sales from the previous week) and `Rolling_Mean_Sales_4w` to capture temporal trends and recent momentum.
    *   **Date & Holiday Features:** We meticulously crafted features like `Weeks_To_Next_Holiday`, `Season` (Winter, Spring, Summer, Fall), and interactions like `Unemployment_Holiday_Combo`.
    *   **External Factors:** We included `CPI_Trend`, `Fuel_Price_Diff`, and Temperature Z-scores to capture economic and environmental context.

*   **Validation Strategy:**
    *   We used **Rolling Window Validation** (Time Series Split) to prevent data leakage.
    *   Train/Test Split 1: Split at 2012-02-01
    *   Train/Test Split 2: Split at 2012-06-01

*   **Model:**
    *   We utilized **XGBoost (Extreme Gradient Boosting)**, a powerful tree-based model known for handling tabular data and non-linear relationships excellent.

---

## 2. Model Performance

The model achieved outstanding results across our validation folds:
...
**Interpretation:** An R² of **0.98** indicates that our model explains **98% of the variance** in weekly sales. This is a very high score, suggesting that our lag features and target encoding successfully captured the sales patterns.

---

## 3. Visual Analysis & Interpretations

We generated several plots to understand *why* the model works so well.
...
To ensure the model isn't a "black box," we used SHAP (SHapley Additive exPlanations).
...
2.  **Waterfall Plot (Local Interpretation)**:
    *   For individual predictions, we can see exactly how features sum up to the final number.
...
3.  **Dependence Plots**:
    ...
    *   We likely see a strong linear relationship between `Lag_Sales` and the SHAP value (higher past sales = higher prediction).

---

## 4. Final Verdict

Our **XGBoost model** combined with **Target Encoding** and **Lag Features** creates a highly accurate forecasting tool for Walmart Sales.
